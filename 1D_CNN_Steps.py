...
parser = ArgumentParser(description='Chem Project ML (1D CNN) - Steps only')
...
if __name__ == '__main__':
    ...
    if PREDMODE:
        ...
        for each_input_file in input_files:
            ...
            # Create unique folder name (EPOCH timestamp + input filename)
            # epoch_time = datetime.datetime.now().strftime("%s") - This sometimes not work
            # Disable EPOCH timestamp (to group files together)
            # epoch_time = calendar.timegm(time.gmtime())
            # dest_dir = str(epoch_time) + '_' + os.path.splitext(each_input_file)[0] + '/'
        ...
        # Need to subtract 1 to range from 0 to 2
        Y = Y - 1
        ...
        model_name = 'STEP_CLASSIFIER_' + training_mode + cur_time + '_' + str(data.shape[0]) + '.h5'
        print('-- Saving model --')
        ...
        conf_matrix_img_filename = 'STEP_CLASSIFIER_' + training_mode + cur_time + '_' + str(data.shape[0]) + '_conf_matrix.png'
